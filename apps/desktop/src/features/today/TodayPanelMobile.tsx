...
import { openQuickEditWindow } from "../time-management/quickEditWindow";
...
export function TodayPanelMobile({ onNavigate }: TodayPanelProps) {
  ...
  const renderTaskItem = (task: Task) => {
    ...
    return (
      <div
        key={task.id}
        className={`today-task-item mobile-task-item${task.completed ? " done" : ""}`}
  ...
  return (
    <div className="today-panel mobile">
      <header className="today-header mobile-header">
        ...
        <div className="today-header-main">
          ...
          <div className="today-meter-legend">
            <span className="lg"><span className="dot t" />任务 <b>{pendingTasks.length}</b></span>
            <span className="lg"><span className="dot h" />习惯 <b>{uncheckedHabits.length}</b></span>
      ...
      {remaining === 0 ? (
        ...
        <div className="today-body mobile-body">
          ...
          <div className="today-mobile-side-section">
              ...
              {todayHabits.length === 0 ? (
                ...
                <>
                  {uncheckedHabits.map((habit) => (
                    <div key={habit.id} className="today-habit-row mobile-habit-row">
                      ...
                      <button type="button" className="today-btn-check mobile-check-btn" onClick={() => handleHabitCheck(habit.id)}>
                        打卡
                      </button>
                    </div>
                  ))}
                  {checkedHabits.map((habit) => (
                    <div key={habit.id} className="today-habit-row done mobile-habit-row">
                      <span className="today-habit-name">{habit.name}</span>
                      <span className="today-habit-streak">连续 {getStats(habit.id, today).currentStreak} 天</span>
            ...
            <div className={`today-side-card today-review-card${reviewWritten ? " written" : ""}`}>
              ...
              {reviewWritten ? (
                ...
                <>
                  ...
                  <button
                    type="button"
                    className="today-btn-review mobile-review-btn"
                    onClick={() => {
                      triggerHaptic('light');
